...
export const register = async (req, res) => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      text: `Welcome to Arqum website. Your Account has been created with email id: ${email} `
    }
...
export const googleCallback = (req, res) => {
  try {
    ...
    // Set token in cookie
    res.cookie("token", token, {
      httpOnly: true,  // client JS se access nahi hoga
      secure: process.env.NODE_ENV === "production", // sirf https par production me
      sameSite: process.env.NODE_ENV === "production" ? "none" : "lax", 
      maxAge: 7 * 24 * 60 * 60 * 1000 // 7 days
    });
...
//Checking if password reset otp ture
